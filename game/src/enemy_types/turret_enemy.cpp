...
std::array<PROJECTILE_SPELL_ID, ranged_enemy_spell_count> tower_enemySpell = {
	PROJECTILE_SPELL_ID::RED_ORB,
};



void TowerEnemy::addComponents(Entity& entity) {

	registry.renderRequests.insert(
		entity,
		{
			TEXTURE_ASSET_ID::TURRET_ENEMY,
	...
	int spell_id = (int)tower_enemySpell[n];
	...
	Animation idle_animation = Animation(TEXTURE_ASSET_ID::TURRET_ENEMY, 4, true); // Asset id, num_frames, is_looping
	...
	animation_manager.animations.insert(std::pair(TEXTURE_ASSET_ID::TURRET_ENEMY, idle_animation));
	// animation_manager.animations.insert(std::pair(TEXTURE_ASSET_ID::RANGED_ENEMY_RUN, run_animation));

	// Play idle animation
	animation_manager.transition_to(animation_manager.animations[TEXTURE_ASSET_ID::TURRET_ENEMY]);
...
void TowerEnemy::step(float elapsed_ms, Entity& self) {
...
void TowerEnemy::burstAttack(Entity& self) {
	Transformation& player_transform = registry.transforms.get(registry.players.entities[0]);
	Transformation& enemy_transform = registry.transforms.get(self);
...
void TowerEnemy::updateEnemyAnimation(Entity& self) {
	Motion& enemy_motion = registry.motions.get(self);
	Transformation& enemy_transform = registry.transforms.get(self);
	...
	animation_manager.transition_to(TEXTURE_ASSET_ID::TURRET_ENEMY);
}
