...
const float BURST_DELAY = 0.37;

BasicRangedEnemy::BasicRangedEnemy(Entity& entity) {
	max_health = 50;
	speed = 175;
	detection_range = 600;
	shooting_range = detection_range * ATTACK_RANGE_FRACTION;
	base_recharge_time = 2350;
...
const int ranged_enemy_spell_count = 1;
std::array<PROJECTILE_SPELL_ID, ranged_enemy_spell_count> ranged_enemy_projectile_spells = {
	PROJECTILE_SPELL_ID::RED_ORB,
};



void BasicRangedEnemy::addComponents(Entity& entity) {
	...
	int n = ((int)random_n == 1) ? 0 : (int)(ranged_enemy_spell_count * random_n);
	int spell_id = (int)ranged_enemy_projectile_spells[n];
	...
	Animation idle_animation = Animation(TEXTURE_ASSET_ID::RANGED_ENEMY_IDLE, 4, true); // Asset id, num_frames, is_looping
	idle_animation.h_frames = 4;
	idle_animation.v_frames = 1;
	idle_animation.frame_rate = ANIMATION_FRAME_RATE / 2;
	Animation run_animation = Animation(TEXTURE_ASSET_ID::RANGED_ENEMY_RUN, 4, true);
	...
	animation_manager.animations.insert(std::pair(TEXTURE_ASSET_ID::RANGED_ENEMY_IDLE, idle_animation));
	animation_manager.animations.insert(std::pair(TEXTURE_ASSET_ID::RANGED_ENEMY_RUN, run_animation));

	// Play idle animation
	animation_manager.transition_to(animation_manager.animations[TEXTURE_ASSET_ID::RANGED_ENEMY_IDLE]);
...
void BasicRangedEnemy::step(float elapsed_ms, Entity& self) {
...
void BasicRangedEnemy::burstAttack(Entity& self) {
	Transformation& player_transform = registry.transforms.get(registry.players.entities[0]);
	Transformation& enemy_transform = registry.transforms.get(self);
	...
	SpellCastManager::castBurstSpell(renderer, spell_slot, enemy_transform.position, direction, self, BURST_COUNT, BURST_DELAY);
}



void BasicRangedEnemy::updateEnemyAnimation(Entity& self) {
	Motion& enemy_motion = registry.motions.get(self);
	Transformation& enemy_transform = registry.transforms.get(self);
	...
	if (glm::length(enemy_motion.velocity) > 0.0) {
		animation_manager.transition_to(TEXTURE_ASSET_ID::RANGED_ENEMY_RUN);
	}
	else {
		animation_manager.transition_to(TEXTURE_ASSET_ID::RANGED_ENEMY_IDLE);
	}
}
