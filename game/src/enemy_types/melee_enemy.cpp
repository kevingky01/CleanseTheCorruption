...
MeleeEnemy::MeleeEnemy(Entity& entity) {
	max_health = 50;
	speed = PLAYER_MOVE_SPEED + 5;
	detection_range = 600;
	shooting_range = 40;
	base_recharge_time = 100;
...
const int melee_enemy_spell_count = 1;
std::array<PROJECTILE_SPELL_ID, melee_enemy_spell_count> melee_enemy_projectile_spells = {
	PROJECTILE_SPELL_ID::FAKE_MELEE,
};



void MeleeEnemy::addComponents(Entity& entity) {

	registry.renderRequests.insert(
		entity,
		{
			TEXTURE_ASSET_ID::MELEE_ENEMY_IDLE,
	...
	int n = ((int)random_n == 1) ? 0 : (int)(melee_enemy_spell_count * random_n);
	int spell_id = (int)melee_enemy_projectile_spells[n];

	damageSpellSlot.spell_id = spell_id; // swap out for different spells
	...
	Animation idle_animation = Animation(TEXTURE_ASSET_ID::MELEE_ENEMY_IDLE, 4, true); // Asset id, num_frames, is_looping
	idle_animation.h_frames = 4;
	idle_animation.v_frames = 1;
	idle_animation.frame_rate = ANIMATION_FRAME_RATE / 2;
	Animation run_animation = Animation(TEXTURE_ASSET_ID::MELEE_ENEMY_RUN, 4, true);
	...
	animation_manager.animations.insert(std::pair(TEXTURE_ASSET_ID::MELEE_ENEMY_IDLE, idle_animation));
	animation_manager.animations.insert(std::pair(TEXTURE_ASSET_ID::MELEE_ENEMY_RUN, run_animation));

	// Play idle animation
	animation_manager.transition_to(animation_manager.animations[TEXTURE_ASSET_ID::MELEE_ENEMY_IDLE]);
...
void MeleeEnemy::step(float elapsed_ms, Entity& self) {
	...
	case ENEMY_STATUS::SHOOTING:
		...
		if (attack_countdown <= 0) {
			...
			SpellCastManager::castSpell(renderer, spell_slot, enemy_transform.position, direction, self);
		}
...
void MeleeEnemy::updateEnemyAnimation(Entity& self) {
	Motion& enemy_motion = registry.motions.get(self);
	Transformation& enemy_transform = registry.transforms.get(self);
	...
	if (glm::length(enemy_motion.velocity) > 0.0) {
		animation_manager.transition_to(TEXTURE_ASSET_ID::MELEE_ENEMY_RUN);
	}
	else {
		animation_manager.transition_to(TEXTURE_ASSET_ID::MELEE_ENEMY_IDLE);
	}
}