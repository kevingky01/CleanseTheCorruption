...
bool RenderSystem::init(GLFWwindow* window_arg)
{
	...
	assert(is_fine == 0 && "gl3w init FAILED");
	...
	std::string font_filename = get_base_path() + "data/fonts/Kenney_Pixel_Square.ttf";
	unsigned int font_default_size = FONT_SIZE;
	initializeFonts(window_arg, font_filename, font_default_size);
	gl_has_errors();
...
/* TODO: init fonts */
void RenderSystem::initializeFonts(GLFWwindow* window, const std::string& font_filename, unsigned int font_default_size) {
...
void RenderSystem::initializeGlGeometryBuffers()
{
	...
	glGenBuffers(instance_buffers.size(), instance_buffers.data()); // for now generates 1 empty buffer
	...
	// The position corresponds to the center of the texture.
	std::vector<TexturedVertex> animated_vertices(4);
	...
	animated_vertices[0].texcoord = { 0.f, 1.f };
	animated_vertices[1].texcoord = { 1.f, 1.f };
	animated_vertices[2].texcoord = { 1.f, 0.f };
	animated_vertices[3].texcoord = { 0.f, 0.f };
	...
	// The position corresponds to the center of the texture.
	std::vector<TexturedVertex> background_verticies(4);
	background_verticies[0].position = { -1.f / 2, +1.f / 2, 0.f };
	background_verticies[1].position = { +1.f / 2, +1.f / 2, 0.f };
	background_verticies[2].position = { +1.f / 2, -1.f / 2, 0.f };
	background_verticies[3].position = { -1.f / 2, -1.f / 2, 0.f };
	background_verticies[0].texcoord = { 0.f, 1.f };
	background_verticies[1].texcoord = { 1.f, 1.f };
	background_verticies[2].texcoord = { 1.f, 0.f };
	background_verticies[3].texcoord = { 0.f, 0.f };

	// Counterclockwise as it's the default OpenGL front winding direction.
	const std::vector<uint16_t> background_indicies = { 0, 3, 1, 1, 3, 2 };
	bindVBOandIBO(GEOMETRY_BUFFER_ID::BACKGROUND, background_verticies, background_indicies);

	// Initialize Particles
	// The position corresponds to the center of the texture.
	std::vector<TexturedVertex> particle_verticies(4);
	particle_verticies[0].position = { -1.f / 2, +1.f / 2, 0.f }; // top left
	particle_verticies[1].position = { +1.f / 2, +1.f / 2, 0.f }; // top right
	particle_verticies[2].position = { +1.f / 2, -1.f / 2, 0.f }; // bottom right
	particle_verticies[3].position = { -1.f / 2, -1.f / 2, 0.f }; // bottom left
	particle_verticies[0].texcoord = { 0.f, 1.f };
	particle_verticies[1].texcoord = { 1.f, 1.f };
	particle_verticies[2].texcoord = { 1.f, 0.f };
	particle_verticies[3].texcoord = { 0.f, 0.f };
	// textured_vertices[0].position = { -1.f / 2, +1.f / 2, 0.f };
	// textured_vertices[1].position = { +1.f / 2, +1.f / 2, 0.f };
	// textured_vertices[2].position = { +1.f / 2, -1.f / 2, 0.f };
	// textured_vertices[3].position = { -1.f / 2, -1.f / 2, 0.f };
	...
	// textured_vertices[3].texcoord = { 0.f, 0.f };
	// Counterclockwise as it's the default OpenGL front winding direction.
	const std::vector<uint16_t> particle_indicies = { 0, 3, 1, 1, 3, 2 }; // TL, BL, TR - TR, BL, BR
	bindVBOandIBO(GEOMETRY_BUFFER_ID::PARTICLE, particle_verticies, particle_indicies);



	// Initialize text
	// The position corresponds to the center of the texture.
	std::vector<TexturedVertex> font_verticies(4);
	font_verticies[0].position = { -1.f / 2, +1.f / 2, 0.f };
	font_verticies[1].position = { +1.f / 2, +1.f / 2, 0.f };
	font_verticies[2].position = { +1.f / 2, -1.f / 2, 0.f };
	font_verticies[3].position = { -1.f / 2, -1.f / 2, 0.f };
	font_verticies[0].texcoord = { 0.f, 1.f };
	font_verticies[1].texcoord = { 1.f, 1.f };
	font_verticies[2].texcoord = { 1.f, 0.f };
	font_verticies[3].texcoord = { 0.f, 0.f };
	// Counterclockwise as it's the default OpenGL front winding direction.
	const std::vector<uint16_t> font_indicies = { 0, 3, 1, 1, 3, 2 };
	bindVBOandIBO(GEOMETRY_BUFFER_ID::FONT, font_verticies, font_indicies);
	...
	/////////////////////////////////////////////////
	// Initialize the health bar geometry NEW
	std::vector<ColoredVertex> bar_vertices(4);
    bar_vertices[0].position = {0.f, 0.f, 0.f};
    bar_vertices[1].position = {1.f, 0.f, 0.f};
    bar_vertices[2].position = {1.f, 1.f, 0.f};
    bar_vertices[3].position = {0.f, 1.f, 0.f};

     
    bar_vertices[0].color = {1.f, 1.f, 1.f};
    bar_vertices[1].color = {1.f, 1.f, 1.f};
    bar_vertices[2].color = {1.f, 1.f, 1.f};
    bar_vertices[3].color = {1.f, 1.f, 1.f};

    std::vector<uint16_t> bar_indices = {0,1,2,0,2,3};

    meshes[(int)GEOMETRY_BUFFER_ID::HEALTH_BAR].vertices       = bar_vertices;
    meshes[(int)GEOMETRY_BUFFER_ID::HEALTH_BAR].vertex_indices = bar_indices;

    bindVBOandIBO(GEOMETRY_BUFFER_ID::HEALTH_BAR, bar_vertices, bar_indices);
	...
	const std::string& vs_path, const std::string& fs_path, GLuint& out_program)
{
	...
	glShaderSource(vertex, 1, &vs_src, NULL);

	GLuint fragment = glCreateShader(GL_FRAGMENT_SHADER);
	// glShaderSource(fragment, 1, &fs_src, &fs_len);
	 glShaderSource(fragment, 1, &fs_src, NULL);
	...
	//!!!
    // glBindAttribLocation(out_program, 0, "in_position");
    // glBindAttribLocation(out_program, 1, "in_texcoord");
    // glBindAttribLocation(out_program, 2, "instance_matrix");
